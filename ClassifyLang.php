<?php
    ...
    $freqEN = array_values($dictEN);
    $freqFR = array_values($dictFR);
    $freqDE = array_values($dictDE);
    $freqES = array_values($dictES);
    $freqPT = array_values($dictPT);
    $freqIT = array_values($dictIT);
    $freqTK = array_values($dictTK);
    $freqSW = array_values($dictSW);
    $freqPL = array_values($dictPL);
    $freqNL = array_values($dictNL);
    $freqDK = array_values($dictDK);
    $freqIS = array_values($dictIS);
    $freqFI = array_values($dictFI);
    $freqCZ = array_values($dictCZ);
    $freqNO = array_values($dictNO);
    $freqET = array_values($dictET);
    $freqGA = array_values($dictGA);
    $freqGL = array_values($dictGL);

    $freqList = array($freqEN, $freqFR, $freqDE, $freqES, $freqPT, $freqIT, $freqTK, $freqSW, $freqPL, $freqNL, $freqDK, $freqIS, $freqFI, $freqCZ, $freqNO, $freqET, $freqGA, $freqGL);

    $langDict = ["EN: "=>0,"FR: "=>0, "DE: "=>0, "ES: "=>0, "PT: "=>0, "IT: "=>0, "TK: "=>0, "SW: "=>0, "PL: "=>0, "NL: "=>0, "DK: "=>0, "IS: "=>0, "FI: "=>0, "CZ: "=>0, "NO: "=>0, "ET: "=>0, "GA: "=>0, "GL: "=>0];
    $langNames = ["EN: ","FR: ", "DE: ", "ES: ", "PT: ", "IT: ", "TK: ", "SW: ", "PL: ", "NL: ", "DK: ", "IS: ", "FI: ", "CZ: ", "NO: ", "ET: ", "GA: ", "GL: "];

    function dotProduct($vec1, $vec2) { //taken (and rearranged) from https://rosettacode.org/wiki/Dot_product#PHP 
        return array_sum(array_map('bcmul', $vec1, $vec2));
      }

    function cosineSimilarity($dot){
        return ($dot / sqrt($dot * $dot));
    }
    ...
    function detectLang($string){
        ...
        global $langNames;
        global $langDict;
        //String preprocessing: lowercasing the string, removing punctuation, whitespace, and digits
        $string = strtolower($string);
        $string = preg_replace('/[^a-z]+/i', '_', $string);
        //Count every instance of a letter in the input string, then estimate frequency
        for($i = 0; $i < count($baseChars); $i++) {
            $occurrence = substr_count($string, $baseChars[$i]);
            array_push($freqTest, $occurrence);
        }
        ...
        arsort($langValues);
        $newLangNames = $langDict;
        array_multisort($newLangNames, SORT_DESC, $langNames);

        echo "Cosine similarity (descending order):<br>";
        for($i = 0; $i <= count($freqList); $i++){

        }
    }

    detectLang("The key() function simply returns the key of the array element that's currently being pointed to by the internal pointer. It does not move the pointer in any way. If the internal pointer points beyond the end of the elements list or the array is empty, key() returns null");
?>
